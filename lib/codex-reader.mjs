...
export function parseCodexSession(file) {
  ...
  for (const rawLine of readFileSync(file, 'utf8').split('\n')) {
    ...
    if (event.type === 'session_meta') {
      ...
      // this file's own session id (matches the filename uuid). Keep the first,
      // not the last, so a new session is not folded into an already-imported
      // parent and skipped as "existing".
