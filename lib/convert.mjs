...
export function buildDshEvents(messages, options = {}) {
  ...
  // 继承源会话标题：latest-wins 的 session/title 事件（fallback 源，
  // 继续对话后 LLM 标题生成器会接管更新）。
...
export function deriveImportTitle(sourceTitle, messages) {
  ...
  if (typeof sourceTitle === 'string') {
    ...
    if (title.length > 0 && !/^new session[- ]/iu.test(title)) return title
  }
  ...
  if (firstUser !== undefined) {
    const text = firstUser.blocks
      ...
      .join(' ')
      .trim()
    if (text.length > 0) return clean(text)
  }
  return undefined
}
